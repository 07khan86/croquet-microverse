class PDFActor {
    setup() {
      ...
      this.listen("ButtonClose", "ButtonClose");
      ...
      this.subscribe(this.id, "buttonPageClose", "ButtonClose");
    }
    ...
    closePdf() {
      //close button icon
      const closeSVG =
        "data:image/svg+xml;base64,";
  
      this.createCard({
        name: "button",
        dataLocation: closeSVG,
    ...
    addButtons() {
      ...
      const buttonSpecs = {
        ...
        down: {
          ...
          position: [0, 0.05],
          rotation: [0, 0, Math.PI],
          translation: [0.07500000000000001, -0.575, 0.05],
        },
        close: {
          svg: closeSVG,
          // svg: closeSVG1,
          // dataLocation: closeSVG1,
          scale: [0.7],
          rotation: [0, 0, Math.PI],
          position: [0, 0.05],
        },
      ...
      const makeButton = (symbol) => {
        ...
        button.call("PDFButton$PDFButtonActor", "setProperties", {
          // id: this.id,
          name: symbol,
          svgScale: scale,
          svgPosition: position || [0, 0],
        });
        return button;
      };
      ...
      ["up", "down", "close"].forEach((buttonName) => {
    ...
    docLoaded(data) {
      ...
      // this.ButtonClose(pdfLocation);
      if (
        pdfLocation === this._cardData.pdfLocation &&
        pdfLocation !== this.measuredDocLocation
      ) {
    ...
    cardDataUpdated(data) {
      if (data.height !== undefined) {
        ...
        const { up, down, close } = this.buttons;
        up.translateTo([-offsetX, -offsetY, depth]);
        down.translateTo([offsetX, -offsetY, depth]);
        // close.translateTo([offsetX, -offsetY, depth]);
        close.translateTo([0.315924337188996, 0.42214015084551715, 0.05]);
        this.say("sizeSet");
      }
    }
    ...
    changePage(increment) {
      ...
      const { page: maxScrollPage, percent: maxScrollPercent } =
      ...
      if (increment === 1) {
        ...
        if (
          page === this.numPages ||
          (page === maxScrollPage && percent === maxScrollPercent)
        )
          page = 1;
        else page++;
      } else {
    ...
    requestScrollPosition({ page, percent }) {
      const { page: newPage, percent: newPercent } = this.normalizeScroll(
        page,
        percent
      );
    ...
    annotateAndAnnounceScroll() {
      ...
      this.scrollState.downAvailable =
        page !== lastPage || percent !== lastPercent;
      this.say("drawAtScrollPosition");
      this.publish(this.parent.id, "updateButtons");
    }
  }
  ...
  class PDFPawn {
    ...
    actorPDFLocation() {
      return this.actor._cardData.pdfLocation;
    }
    hasLatestPDF() {
      return !!this.pdf && this.pdfLocation === this.actorPDFLocation();
    }
    ...
    async measureDocument() {
      ...
      const gapPercent =
        firstWidth > firstHeight ? 2 : firstWidth === firstHeight ? 1.5 : 1;
      this.adjustCardSize(
        firstWidth,
        firstHeight,
        gapPercent,
        !actorHasMeasurements
      ); // includes setting pageGap, which we need in order to render anything
      ...
      if (!actorHasMeasurements) {
        ...
        this.say("docLoaded", {
          pdfLocation,
          pageGapPercent: gapPercent,
          numPages,
          maxScrollPosition: lastScroll,
        });
      }
    }
    ...
    drawAtScrollPosition() {
      ...
      meshPool.forEach((mesh) => {
        if (!this.visiblePages[mesh.lastAssignedPage])
          delete mesh.lastAssignedPage;
      });
      ...
      while (true) {
        ...
        if (renderResult) {
          ...
          let pageMesh;
          let existingIndex = meshPool.findIndex(
            (mesh) => mesh.lastAssignedPage === p
          );
          if (existingIndex < 0)
            existingIndex = meshPool.findIndex((mesh) => !mesh.lastAssignedPage);
          ...
          const pageHeight = Math.min(
            (1 - imageTop) * fullPageHeight,
            cardHeight - shownHeight - topGap
          );
          const imageBottom = imageTop + pageHeight / fullPageHeight;
          const geo = (pageMesh.geometry = new Microverse.THREE.PlaneGeometry(
            cardWidth,
            pageHeight
          ));
          ...
          if (!pageEntry.texture)
            pageEntry.texture = new Microverse.THREE.Texture(renderResult);
    ...
    updateButtons() {
      ...
      const { upAvailable, downAvailable, closeAvailable } = scrollState;
    ...
    makePageMesh() {
      const { cardWidth, cardHeight } = this;
      const pageGeometry = new Microverse.THREE.PlaneGeometry(
        cardWidth,
        cardHeight
      );
      const pageMaterial = new Microverse.THREE.MeshBasicMaterial({
        color: "#fff",
    ...
    startRendering(pageNumber) {
      ...
      renderTask.promise.then(
        () => {
          ...
          pageEntry.renderResult = context.getImageData(
            0,
            0,
            viewport.width,
            viewport.height
    ...
    squareCornerGeometry(width, height, depth) {
      ...
      let extrudePath = new Microverse.THREE.LineCurve3(
        new Microverse.THREE.Vector3(0, 0, z),
        new Microverse.THREE.Vector3(0, 0, -z)
      );
    ...
    onPointerMove(p3d) {
      ...
      const percentChange =
        ((p3d.xyz[1] - this.pointerDownY) / this.cardHeight / yScale) * 100;
      this.pointerDragRange = Math.max(
        this.pointerDragRange,
        Math.abs(percentChange)
      );
      this.say("requestScrollPosition", {
        page,
    ...
    onPointerUp(p3d) {
      ...
      if (this.pointerDragRange < 2 && Date.now() - this.pointerDownTime < 250)
        this.changePage(1);
    }
    ...
    onPointerWheel(evt) {
      ...
      const WHEEL_SCALE = 4;
      let percent =
        ((this.cumulativeWheelDelta * WHEEL_SCALE) / this.TEXTURE_SIZE) * 100;
    ...
    buttonPressed(buttonName) {
      ...
      else if (buttonName === "close") this.ButtonClose();
  
      console.log("===buttonPressed buttonName:", buttonName);
    }
    ...
    teardown() {
      console.log("PDFPawn teardown this :", this.textMesh);
      this.cleanupShape();
  ...
  class PDFButtonActor {
    setup() {
      this.addEventListener("click", "onClickButton");
    }
    onClickButton() {
      // this.destroy();
      if (this.buttonName === "close") {
        this.parent.destroy();
      }
      console.log("this this :", this.buttonName === "close");
    }
  
    setProperties(props) {
      // if (props.name === "close") {
      //   // this.svgPosition = [-4, -18];
      //   (translation = [0.315924337188996, 0.42214015084551715, 0.05]),
      //     (scale = [0.055999999999999994, 0.055999999999999994, 1]),
      //     (rotation = [0, 0, 1, 6.123233995736766e-17]);
      // }
      console.log("setProperties :", props);
      console.log("setProperties :", props.name === "close");
  ...
  class PDFButtonPawn {
    setup() {
      ...
      this.subscribe(this.id, "updateButtons", "updateState");
    ...
    svgLoaded() {
      ...
      const segments = 32;
      const geometry = new Microverse.THREE.CylinderGeometry(
        radius,
        radius,
        depth,
        segments
      ...
      const material = new Microverse.THREE.MeshBasicMaterial({
        color: 0xa0a0a0,
        side: Microverse.THREE.DoubleSide,
        transparent: true,
        opacity,
      ...
      hittableMesh.raycast = (...args) =>
        this.shape.visible ? hittableMesh._baseRaycast(...args) : false;
    ...
    tapped() {
      if (this.actor.buttonName == "close") {
        this.publish(
          this.parent.actor.id,
          // this.id,
          "buttonPageClose"
          // this.pdfdataId === undefined
          //   ? "3unewzSR3FqBRu3nDd3X7-KHXcPyICpaZYrvkwFob-AQHQEBBQZPWloTHBkQBlsABlsWBxoEABABWxwaWgBaDyABAiU6DzMAOiZEPhwSOC8cQEZMDTMxMk1FR1ocGlsWBxoEABABWxgcFgcaAxAHBhBbGRoWFBkREAMREBMUABkBWkwBPjkTFzkvPh0fEUw0DUQAQxg3ETAyJSE6Nh1DRkEtMExAGBwYHhxCAQZaERQBFFoyJToBBCEeQUAGFBxGHjc2AFgtQD8HEUQFPg8sIS03GxlAWBonB0YPHioC"
          //   : ""
        );
        // hardcoded id
        // this.ButtonClose(
        ...
        this.publish(
          this.parent.actor.id,
          "buttonPageChange",
          this.actor.buttonName === "down" ? 1 : -1
        );
      }
    }
    ...
    ButtonClose(dat) {
      ...
      console.log("close button::" + dat);
      console.log("this.pdfdataId:" + this.pdfdataId);
      console.log("this.id:" + this.id);
      console.log(" this.parent.actor.id:" + this.parent.actor.id);
      console.log("this.parent after close:", this.parent);
    }
  }
