...
class BitcoinTrackerPawn {
    ...
    */
    fetchHistory() {
        ...
        return fetch(`https://www.retailjewels.com/RetailJewelsAPI/service.asmx/GetThumbnailProductSearch?Price=&Caratweight=&StoneShapes=&Stonesettings=&JewelleryCollections=&GroupId=8ed5c7da-6a9e-4ba7-8be2-ea4f18c666d3&NewArival=0&SpecialOffer=0&SortingBy=0&PageIndex=1&EternityCollection=&BestSeller=0&ProductRating=0`).then((response) => {
            return response.json();
        }).then((json) => {
            // const prices = json.map(item => item.MWImage);
            // const RetailPrice = json.map(item => item.RetailPrice);
            // console.log("fetched prices RetailsPrices", RetailPrice[0]);
            // var data = [{
            //     'prices' : prices[0],
           ...
           // console.log("publishing %s latest prices", prices.length);
            console.warn("json data", json[0]);
            
            this.say("BTC-USD-history", json[0]);
        });
    }
    ...
    */
    ...
    async onBTCUSDChanged() {
        //console.log("changed");
        // this is called on all views, not just the elected one
        // console.warn('RetailPriceRetailPrice', this.data)
        let base_image = new Image();
       var productData = await fetch(`https://www.retailjewels.com/RetailJewelsAPI/service.asmx/GetThumbnailProductSearch?Price=&Caratweight=&StoneShapes=&Stonesettings=&JewelleryCollections=&GroupId=8ed5c7da-6a9e-4ba7-8be2-ea4f18c666d3&NewArival=0&SpecialOffer=0&SortingBy=0&PageIndex=1&EternityCollection=&BestSeller=0&ProductRating=0`).then((response) => {
        return response.json();
        }).then((json) => {
            base_image.src = './assets/images/earthbase.png';
           // base_image.src = 'https://www.retailjewels.com/Admin/UploadImages/HighResolutionImages/white/B131_white.jpg';
            return json[0]
        })
        ...
        ctx.font = "30px Arial";
        ctx.fillText("price: " +productData.RetailPrice, this.canvas.width - 40, 85);
        
        ctx.textAlign = "center";
        ctx.font = "25px Arial";
        ctx.textBaseline = "top";
        //ctx.fillText("$" + amount.toFixed(2), this.canvas.width / 2, 100); //50+this.canvas.height/2);
        ctx.fillText("Title: "+productData.AffTitle, this.canvas.width / 2, 100); //50+this.canvas.height/2);
