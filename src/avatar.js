...
const EYE_HEIGHT = 3.380;// camera height value
...
export class AvatarActor extends mix(CardActor).with(AM_Player) {
    ...
    nameSet({o, v,}) {
        if (o !== v){ 
            ...
            let names = v; // Initial value of Uma
let namesMenuList = document.getElementById("names_menu-list");

function addUmaToList() {
  let listItem = document.createElement("li");
  listItem.textContent = names;
  namesMenuList.appendChild(listItem);
}

addUmaToList()
           
            this.publish("playerManager", "nameChanged")
    ...
    startFalling() {
        //the below line is responsible for wall crossings (to make avatar to pass through the wall set this.fall false)
        this.fall = true;
    }
...
export class AvatarPawn extends mix(CardPawn).with(PM_Player, PM_SmoothedDriver, PM_ThreeVisible, PM_ThreeCamera, PM_Pointer) {
    ...
    showNumbers() {
        ...
        let suffix = here === 1 ? "MP-World" : "MP-Worlds";
        let tooltip = `${here} ${here === 1 ? "MP-World is" : "MP-Worlds are"} in this world`;
        if (here !== total) {
            let watching = total - here;
            tooltip += `, ${watching} ${watching === 1 ? "MP-World has" : "MP-Worlds have"} not entered yet`;
            total = `${here}+${watching}`;
        }
        if (manager.presentationMode) {
            ...
            tooltip = `${followers} ${followers === 1 ?  "MP-World" : "MP-Worlds"} in guided tour, ${tooltip}`;
        } else {
    ...
    makeCardSpecFrom(configuration, actor, avatarName) {
        ...
        if (avatarType === "wonderland" || !configuration.type) {
            ...
            //the code shouls go shownumber of functions
            // let manager = this.actor.service("PlayerManager");

            // let players_name = manager.playersInWorld()

      
            // const menuList = document.getElementById("names_menu-list");
          
            // // Clear the existing content of the menuList
            // menuList.innerHTML = "";
          
            // // Create and append new list items for each user in the updatedPlayers array
            // players_name.forEach((user, index) => {
            //   const listItem = document.createElement("li");
            //   listItem.textContent = user._name;
            //   menuList.appendChild(listItem);
            // });

                    

            // const parentElement = document.getElementById("worldMenuBtn1");

            // // Find the existing list
            // const existingList = parentElement.querySelector("ul");

            // if (existingList) {
            // // Iterate over the list items in the existing list
            // const listItems = existingList.querySelectorAll("li");
            // listItems.forEach((item) => {
            //     // Check if the text content matches the value you want to update
            //     if (item.textContent === this.actor._name) {
            //     // Update the text content with the new value
            //     item.textContent = configuration.nickname;
            //     }
            // });
            // }
            ...
            let options = {
                ...
                dataLocation: maybeDataLocation || configuration.avatarURL,
                modelType: "glb",
                type: "3d",
    ...
    showSettingsMenu() {
        ...
        promise.then(changed => {
            if (changed) {
                ...
                //code started to update naming list 

                ///////////////////////////////////////////

                // const parentElement = document.getElementById("names_menu");
                // const existingList = parentElement.querySelector("ul");
                ...
                // const existingList = parentElement.querySelector("ul");
                // if (existingList) {
                //   parentElement.removeChild(existingList);
                //   const newList = document.createElement("ul");
                //   newList.id = "names_menu-list";
