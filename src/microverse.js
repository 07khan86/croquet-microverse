...
class MyPlayerManager extends PlayerManager {
    ...
    playerDestroyed(_player) {
        ...
        console.log(_player._name)

        var name = "uma";
     //To delete the player name from name list ../
// Check if the name exists in the list
        var nameList = document.getElementById("names_menu-list");
        var listItems = nameList.getElementsByTagName("li");
        for (var i = 0; i < listItems.length; i++) {
        if (listItems[i].textContent.trim() === _player._name) {
            // Remove the list item from the DOM
            listItems[i].parentNode.removeChild(listItems[i]);
            break; // Exit the loop since the item has been found and removed
        }
        }
        //....delete name list  code end //
        this.publish("playerManager", "playerCountChanged");
    }
}
